...
This is a Decentralized Exchange built with smart contracts powered by Ethereum. It basically consists in a platform where the user can exchange an ERC-20 token called SRS Token by Ether in a secure and descentralized manner.
...
```
$ git clone https://github.com/solaris0724/decentralized-exchange.git
```
...
This project consists in a decentralized exchange to allow users making p2p trades between Ether and a ERC-20 token called SRS Token. It includes:

- A smart contract which contains an ERC-20 token called SRS Token
...
Here is where the user can deposit / withdraw Ether or SRS Token in the Exchange. It also reflects the balances, not only exchange balances but also wallet ballances,
...
The Price Chart reflects the price of the SRS Token via candlestick chart.
